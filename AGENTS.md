...
- 공휴일과 비행편 정보는 주차 수집과 분리해 조회하되, 시계열 차트에서는 같은 시간축 위에 이해하기 쉽게 표시해야 한다.
...
- 시계열 차트는 실제 주차 관측 구간과 미래 비행편 표시 구간을 구분해서 보여준다.
- 공휴일 배경, 비행편 마커 hover/click 같은 상호작용을 추가하면 컴포넌트 테스트를 함께 갱신한다.
